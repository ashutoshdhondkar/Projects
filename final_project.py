...
from openpyxl.chart import PieChart3D,Reference,BarChart3D
import re
from bs4 import *
...
class SEO:
    ...
    def WriteFrequency(self,book):
        ...
        for y in self.count:
                ...
                i=i+1
        self.wbk.save(book)
    def DrawChart(self,book):
        words = Reference(self.wsht,min_col=3,min_row=2,max_row=len(self.data)+1)
        freq = Reference(self.wsht,min_col=4,min_row=2,max_row=len(self.data)+1)
        print("\n"*5)
        print(words)
        print(freq)
        print("\n"*5)
        pie = PieChart3D()
        pie.add_data(freq,titles_from_data=True)
        pie.set_categories(words)
        pie.title="Word Frequency in given URL"
