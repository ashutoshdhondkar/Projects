    ...
    def PrintNew(self):
        if(self.ticket_number.get()):
            if(self.fullname.get() and self.department.get() and self.shift.get() and self.memo.get()):
                ...
                self.PrintData()
                fp = open("Logs/"+str(self.td.date())+".txt",'a')
        ...
        if(self.ticket_number.get()):
            if(NewDataBase.CheckTicket(self.ticket_number.get())):
                ...
                if(self.dictionarydata['flag']==0):
                    ...
                    self.td = datetime.datetime.now()
                    self.check_out.set("%0.2d:%0.2d"%(self.td.hour,self.td.minute))
...
#********************************************************************************************************
    def In(self):
        ...
        if(self.UploadInfo()):
            ...
            #####
            self.choices = ['CARD FORGOT','CARD LOST','CARD LOCKED','OTHER']
    ...
    def Out(self):
        ...
        if(self.UploadInfo()):
            ...
            Label(self.root,textvariable=self.memo1,font=RestrictedEntry.allfont,bg=RestrictedEntry.bg).place(x=800,y=350)
            ...
            self.memo1.set(self.dictionarydata['reason_for_memo'])
            ...
            if(self.check==0):  #if the user is outside the premises bt admin hit in button so to avoid errors care is taken
                ...
                self.time.set("%0.2d:%0.2d"%(self.td.hour,self.td.minute))   # set check_out date
    ...
    def update(self):
        if(self.ticket_number.get()):
            DataBase.updateAdmin(self.ticket_number.get(),self.memo.get(),self.remark.get('1.0','end'))
    ...
    def UploadInfo(self):
        ...
        #print(ticket)
        try:
            self.path="D:/Apache Software Foundation/Tomcat 9.0/webapps/cenrayid/WEB-INF/classes/photo/"+self.ticket_number.get()+".jpg"
...
class DataBase:
    ...
    def CreateTable(cls):
        pass
    @classmethod
    def update(cls,ticket_number,memo,remark,date,time,shift,dept_code): #updating only in time
        #print(memo,remark,date,time,cardLost)
        cls.db.execute('''UPDATE LABOURS SET
        
                            Remark=?,check_in=?,check_out=null,date_in=?,date_out=null,reason_for_memo=?,
                            flag=1,shift=?,department_code=?  WHERE TicketNo=?; ''',(remark,time,date,memo,shift,dept_code,ticket_number))
        cls.db.commit()
    @classmethod
    def updateAdmin(cls,ticket_number,memo,remark):
        cls.db.execute('''UPDATE LABOURS SET
    ...
    def load(cls,ticket_number):
        ...
        for x in data:
            ...
            dictionarydata['reason_for_memo'] = x[3]
            dictionarydata['department'] = x[4]
            dictionarydata['check_in'] = x[5]
            dictionarydata['check_out'] = x[6]
            dictionarydata['date_in'] = x[7]
            dictionarydata['flag'] = x[8]
            dictionarydata['date_out'] = x[9]
            dictionarydata['shift'] = x[10]
            dictionarydata['department_code'] = x[11]
